...
export const ScreenCaseStudy: React.FC<ScreenCaseStudyProps> = () => {
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black select-none flex flex-col justify-between">
      ...
      {/* 2. Structured Layout Container across full viewport width */}
      <div className="relative z-20 w-full h-full flex flex-col justify-between px-8 sm:px-12 lg:px-16 xl:px-20 py-8 sm:py-10">
        {/* Top Header: Title pushed cleanly to the left */}
        <header className="w-full pt-1">
          <div className="space-y-1 max-w-xl">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif tracking-[0.16em] text-[#EDE6DC] font-normal uppercase leading-[1.12]">
              HÀNH TRÌNH
            </h2>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif tracking-[0.16em] text-[#EDE6DC] font-normal uppercase leading-[1.12]">
              CỦA CHIẾC HỘP
        ...
        <div className="w-full flex-1 flex items-center justify-between my-auto">
          {/* Left area open for the 3D artwork */}
          <div className="hidden lg:block flex-1" />

          {/* Right Column: Description sitting immediately above the steps */}
          <div className="w-full max-w-md lg:max-w-lg flex flex-col justify-center space-y-7 sm:space-y-8 pr-2 lg:pr-8 xl:pr-12">
            {/* Description placed directly above step 01 */}
            <p className="text-xs sm:text-sm font-sans text-white/50 leading-relaxed text-left font-light max-w-md">
              Đây là hành trình tạo tác của sản phẩm đang được khám phá. Tùy vào sản phẩm và kỹ thuật, các công đoạn có thể khác nhau.
            </p>

            {/* 6 Steps List */}
            <div className="flex flex-col space-y-5 sm:space-y-6 lg:space-y-6">
              {steps.map((step) => (
                <div key={step.id} className="flex items-center gap-4 sm:gap-5">
                  {/* Circular Badge with number */}
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border border-white/30 flex items-center justify-center text-xs font-mono tracking-widest text-white/70 shrink-0">
                    {step.id}
                  </div>

                  {/* Title */}
                  <span className="text-sm sm:text-base font-sans tracking-[0.18em] uppercase text-white/80 font-light">
                    {step.title}
                  </span>
                </div>
              ))}
